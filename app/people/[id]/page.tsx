...
export default async function PersonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><div className="container">
    ...
    <div className="two-column"><section className="card"><h2>Nickname options</h2><div className="nickname-list">
      ...
      {rows.map((row: any) => <div className="nickname-row" key={row.nickname_id}><div className="nickname-main"><strong>{row.nickname}</strong>{row.is_final && <span className="badge">Final nickname</span>}</div><div className="button-row">{settings?.results_visible && <span className="vote-count">{row.vote_count} votes</span>}<VoteButton personId={id} nicknameId={row.nickname_id} selected={row.has_voted} disabled={!settings?.voting_open || Boolean(person.finalized_nickname_id)} /></div></div>)}
